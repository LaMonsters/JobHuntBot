...
The header's **"＋新增投递" (Add application)** button opens a form for adding a lead or a confirmed application without editing CSV by hand. Every card has an **"编辑投递" (Edit application)** action for changing company, role, status, dates, resume variant, follow-up, notes, and optional submission evidence. Saving a confirmed `Submitted` / `Offer` / `Rejected` record also upserts its matching row in `application_log.csv`; the form requires an explicit confirmation that it was a real submission, not a saved draft.
...
You can still open the CSVs directly in Excel/Google Sheets/Numbers. The dashboard writes the same files through its local server, so click **刷新** after outside edits. It does not upload data or submit applications to any job site.
...
顶部的 **“＋新增投递”** 会打开新增表单；每张岗位卡片展开后都有 **“编辑投递”**。表单可以维护公司、岗位、状态、日期、简历版本、下一步和备注等字段；当状态为 `Submitted`、`Offer` 或 `Rejected` 时，还会在确认后同步 `application_log.csv`。系统要求明确确认是真实提交过的申请，不会把收藏或网申草稿算成已投递。

你也可以直接用 Excel/Google Sheets/Numbers 打开这些 CSV；看板和表格使用的是同一份数据，外部修改后点击“刷新”即可。所有写入都发生在本机，不会上传数据，也不会代替你向招聘网站提交申请。
