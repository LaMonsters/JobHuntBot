...
(async () => {
  try {
    ...
    const captured = { kind: 'offertrack-job', version: 1, job: { company: '采集兼容公司', role: '测试工程师', city: '北京', sourceUrl: 'https://example.com/jobs/captured', jd: '原插件的岗位描述', jobType: '技术岗', resumeVersion: '旧版名称' } };
    await page.goto(base + '/dashboard.html#offertrack-import=' + encodeURIComponent(JSON.stringify(captured)));
    await page.locator('#record-dialog').waitFor();
    assert.equal(await page.locator('#field-jd').inputValue(), captured.job.jd);
    assert.equal(page.url().includes('offertrack-import'), false);
    await page.locator('#record-form button[type="submit"]').click(); await page.locator('#record-dialog').waitFor({ state: 'hidden' });
    assert.equal((await saved()).length, 1); assert.equal((await saved())[0].stage, 'planned');
    await page.locator('#import-job-file').setInputFiles({ name: 'captured.json', mimeType: 'application/json', buffer: Buffer.from(JSON.stringify(captured)) });
    await page.locator('#record-form button[type="submit"]').click();
    await page.locator('#form-error').waitFor(); assert.equal((await saved()).length, 1);
    await page.locator('#record-dialog [data-close]').last().click();
    const local = await context.newPage(); local.on('pageerror', error => errors.push(error.message));
    await local.goto(require('url').pathToFileURL(path.join(root, 'dashboard.html')).href + '#offertrack-import=' + encodeURIComponent(JSON.stringify(captured)));
    await local.locator('#record-dialog').waitFor();
    await local.locator('#record-form button[type="submit"]').click(); await local.locator('#record-dialog').waitFor({ state: 'hidden' });
    await local.reload();
    assert.equal(await local.locator('body').getAttribute('data-page'), 'applications');
    await local.locator('#search').fill('采集兼容公司'); assert.equal(await local.locator('.job-card').count(), 1);
    await local.locator('.sidebar [data-nav="resumes"]').click();
    assert.ok((await local.locator('#resume-service-message').innerText()).includes('打开完整工作台'));
    await local.close();
    passed('原入口兼容：插件链接导入、重复岗位保护、file:// 保存与本地简历服务提示');
